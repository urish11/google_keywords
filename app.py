...
# Number input with controlled width
count_gpt_kws = st.number_input("Enter a number:", value=0)
...
if st.button("Fetch Keyword Ideas"):
    with st.spinner("Fetching data..."):
        ...
        if enable_gpt_kws:
            gpt_kws = chatGPT(f"write more {str(count_gpt_kws)} diverse keywords for search arb with high intent and high CPC, return JUST THE PLAIN TXT the new keywords each spereted with \n for: {keywords_input}")
